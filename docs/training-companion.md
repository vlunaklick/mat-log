...
Set the optional server-side `OPENROUTER_API_KEY` in `server/.env` for local use, or in the Coolify Application environment for production, to enable the Coach fallback after deployment. Gemini remains primary. HTTP 408/429/5xx, retryable network errors and timeouts trigger one generation attempt through `openrouter/free`. Each provider gets 60 seconds when fallback is configured; without a key Gemini keeps its 120-second timeout and one retry. Authentication errors, invalid requests, invalid output and database failures do not trigger fallback. Audio transcription still uses Gemini only.

The fallback model is fixed to OpenRouter's free router and requires support for all request parameters. All responses still pass the local Coach schema. Read-only history/catalog tools can run again on fallback, but messages, drafts and proposals are persisted only by the existing route after generation succeeds. If both providers fail, the saved user message can be retried. Free routing is subject to OpenRouter's quota and availability and may select different models between requests. No paid fallback is configured. See [free routing](https://openrouter.ai/docs/guides/routing/routers/free-router) and [limits](https://openrouter.ai/docs/api_reference/limits).
