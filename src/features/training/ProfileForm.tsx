...
function ProfileEditor({ initial }: { initial: TrainingState }) {
  ...
  const currentBelt = profile.belts[profile.belts.length - 1];
  return (
    <Card>
      <CardHeader>
      ...
      <CardContent>
        <FieldGroup>
          ...
          {currentBelt && (
            <p className="text-sm text-muted-foreground">
              Cinturón: {BELT_LABELS[currentBelt.belt]}
              {currentBelt.date ? ` (desde ${formatDate(currentBelt.date)})` : ""}
            </p>
          )}

          <Disclosure summary="Datos físicos">
          ...
          <Disclosure summary="Estilo y objetivos">
            <FieldGroup>
              {(
                [
              ...
              {profile.breaks.map((b, i) => (
                <FieldGroup key={i}>
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <Field>
                      <FieldLabel htmlFor={`break-end-${i}`}>Vuelta</FieldLabel>
                      <Input
                        id={`break-end-${i}`}
                        type="date"
                        value={b.end ?? ""}
                      ...
                      {i === 0 && (
                        <FieldDescription>
                          Dejala vacía si sigue la pausa.
                        </FieldDescription>
                      )}
                    </Field>
                  </div>
