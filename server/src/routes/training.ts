...
export const trainingRoute = new Hono<AppEnv>()
  ...
  .put("/state", async (c) => {
    const { payload, revision } = z
      .object({ payload: payloadSchema, revision: revisionSchema })
      .parse(await c.req.json());
    return c.json(
      await db.transaction((tx) =>
        applyPayload(tx, c.get("userId"), payload, revision),
      ),
    );
  })
  .get("/drafts", async (c) =>
    c.json(
      await db
        .select()
        .from(schema.drafts)
        .where(eq(schema.drafts.userId, c.get("userId")))
  ...
  .get("/drafts/:id", async (c) => {
    const [row] = await db
      ...
      .where(
        and(
          eq(schema.drafts.userId, c.get("userId")),
          eq(schema.drafts.id, c.req.param("id")),
        ),
      );
  ...
  .put("/drafts/:id", async (c) => {
    const { revision, data, questions } = z
      .object({
    ...
    const [row] = await db
      ...
      .set({ data, questions, revision: revision + 1, updatedAt: Date.now() })
      .where(
        and(
          eq(schema.drafts.userId, c.get("userId")),
          eq(schema.drafts.id, c.req.param("id")),
      ...
      .returning();
    if (!row)
      fail(409, "El borrador cambió o ya fue confirmado. Recargá la página.");
    return c.json(row);
  })
  .post("/drafts/:id/confirm", async (c) => {
    const { revision } = z
      ...
      .parse(await c.req.json());
    return c.json(
      await confirmDraft(c.get("userId"), c.req.param("id"), revision),
    );
  })
  ...
  .get("/proposals", async (c) =>
    c.json(
      await db
        ...
        .where(eq(schema.proposals.userId, c.get("userId")))
  ...
  .post("/proposals/:id/:action", async (c) => {
    ...
    return c.json(
      await db.transaction(async (tx) => {
        await ensureState(c.get("userId"), tx);
        await tx
          .select()
          .from(schema.trainingState)
          .where(eq(schema.trainingState.userId, c.get("userId")))
          .for("update");
        const [proposal] = await tx
          ...
          .where(
            and(
              eq(schema.proposals.userId, c.get("userId")),
              eq(schema.proposals.id, c.req.param("id")),
            ),
          )
          .for("update");
        ...
        if (action === "accept")
          await applyPayload(
            tx,
            c.get("userId"),
            payloadSchema.parse(proposal.payload),
