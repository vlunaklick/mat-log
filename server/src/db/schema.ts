...
import type { Profile, Goal, Gameplan, DraftData, ProposalPayload, TechniqueEvidence } from "../../../src/lib/training.ts";
...
export const trainingSessions = pgTable(
  "training_session",
  {
    ...
    evidence: jsonb("evidence").$type<TechniqueEvidence[]>().notNull().default([]),
...
export const drafts = pgTable("training_draft", {
  id: text("id").primaryKey(),
  userId: text("user_id").notNull().references(() => user.id, { onDelete: "cascade" }),
...
export const proposals = pgTable("coach_proposal", {
  id: text("id").primaryKey(),
  userId: text("user_id").notNull().references(() => user.id, { onDelete: "cascade" }),
  conversationId: text("conversation_id").references(() => conversations.id, { onDelete: "set null" }),
  title: text("title").notNull(), reason: text("reason").notNull(),
  payload: jsonb("payload").$type<ProposalPayload>().notNull(),
  baseRevision: integer("base_revision").notNull(),
  status: text("status").$type<"pending" | "accepted" | "dismissed">().notNull().default("pending"),
  createdAt: bigint("created_at", { mode: "number" }).notNull(),
...
export const chatMessages = pgTable(
  ...
  (t) => [index("chat_message_user_created_idx").on(t.userId, t.createdAt), index("chat_message_conversation_idx").on(t.userId, t.conversationId, t.createdAt)],
);
