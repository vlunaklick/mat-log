...
Un borrador no es evidencia confirmada. Puede consultarse, pero no cuenta para progreso. Conservá las preguntas sin responder. Si hay un objetivo activo de la modalidad de la clase y falta el resultado, preguntá si pudo intentarlo y guardá la respuesta en goalNotes. Preguntá cantidades solo cuando sirvan para ese objetivo. Máximo dos preguntas importantes por respuesta salvo necesidad expresa. Se puede guardar incompleto.
...
export async function respond(
  ...
  const [state, sessionRows, techniqueRows, history, draftRows] =
    await Promise.all([
      getState(userId),
      db
        .select()
  ...
  const selected = selectedDraftId
    ? (draftRows.find((d) => d.id === selectedDraftId) ??
      (
        await db
          .select()
          .from(schema.drafts)
          .where(
            and(
              eq(schema.drafts.id, selectedDraftId),
              eq(schema.drafts.userId, userId),
            ),
          )
      )[0])
    : null;
  if (selectedDraftId && (!selected || selected.status !== "draft"))
    throw new Error("El borrador ya fue confirmado o no existe.");
  const model =
    modelOverride ??
