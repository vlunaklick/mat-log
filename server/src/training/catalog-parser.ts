...
export function inferPosition(text: string): Position {
  ...
  const rules: Array<[RegExp, Position]> = [
    [/half|media/, "Half Guard"],
    [/full.guard|closed|cerrada/, "Closed Guard"],
    [/mount|montada/, "Mount"],
    [/side.control|side.ctrl|lateral|north.south/, "Side Control"],
    [/back|rear.naked|espalda/, "Back"],
    [/turtle|tortuga/, "Turtle"],
    [/leg.lock|heel|50.50|ashi|entangle/, "Leg Entanglements"],
    [/pass|knee.cut|knee.slice/, "Guard Passing"],
    [
      /standing|takedown|throw|single.leg|double.leg|derribo/,
      "Standing / Takedowns",
    ],
    [/guard|butterfly|de.la.riva/, "Open Guard"],
    [/escape|shrimp/, "Escapes"],
  ];
...
export function inferType(s: string): TechniqueType {
  ...
  if (
    /choke|armbar|arm.bar|kimura|americana|heel.hook|kneebar|guillotine|triangle|omoplata/i.test(
      s,
    )
  )
    return "submission";
...
export function parseGrappleMap(text: string, commit: string): CatalogEntry[] {
  ...
  return blocks.flatMap((b, i) => {
    ...
    if (
      ...
      /^(\.\.\.|wip)$/i.test(name) ||
      b.meta.some((l) =>
        /^todo:.*(stub|remove|replace|wip|need source|find better source)/i.test(
          l,
        ),
      )
    )
    ...
    const tags = b.meta
      ...
      .flatMap((l) => l.slice(5).trim().split(/\s+/));
    const refs = b.meta
    ...
    return [
      {
        ...
        position: inferPosition(searchable),
        type: inferType(searchable),
        style: "nogi" as const,
        description: tags.map((t) => t.replaceAll("_", " ")).join(" · "),
        tags,
        source: "GrappleMap" as const,
        sourceUrl: `https://github.com/Eelis/GrappleMap/blob/${commit}/GrappleMap.txt`,
        references: refs,
