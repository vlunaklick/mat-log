...
- **Type**: Inter Variable. Headings 650 (`text-h1`, `text-h2`, `text-h3`, `text-title` utilities), body 450 (default), light 300 lead (`text-lead`), labels `text-label`. Sentence case, headlines short and without a final period ("Diario", "Editar clase"). No letter-spacing, no all-caps ("Gi", "No-gi", never "GI").
- **Copy**: all visible text in Rioplatense Spanish with voseo ("registrá", "podés"), including placeholders, aria-labels, toasts and errors. BJJ terms used as-is in Argentina stay in English ("gi", "no-gi", "roll", "drill", "gameplan"). Stored values stay in English; display them through `src/lib/labels.ts`. Prefer less text: no descriptions that repeat the title, no filler.
...
- **Empty states** use `Empty` (or `Blank` from `src/features/training/shared.tsx`) with one concrete action; callouts use `Alert`; confirmations use `AlertDialog`; toasts via `toast()` from `sonner`.
...
| Destructive | `Button variant="destructive"` inside an `AlertDialog` flow (`ConfirmDelete`), placed last and apart from the primary action |
